...
public class Jaro extends CheckStringDizionario {
	...
	protected boolean singleCheck(String parolaDaConfrontare, String parolaNelDizionario) {
		...
		return this.compare(parolaDaConfrontare, parolaNelDizionario)>0.9;
	}
	...
	public float compare(final String a, final String b) {
		...
		final int halfLength = Integer.max(0, Integer.max(a.length(), b.length()) / 2 - 1);
	...
	/*
	 * Returns an array of characters from a within b. A character in b is counted
	 * as common when it is within separation distance from the position in a.
	 */
	private static int[] getCommonCharacters(final char[] charsA, final char[] charsB, final int separation) {
		final int[] common = new int[Integer.min(charsA.length, charsB.length)];
	...
	private static int indexOf(char character, char[] buffer, int fromIndex, int toIndex, boolean[] matched) {
		...
		for (int j = Integer.max(0, fromIndex), length = Integer.min(toIndex, buffer.length); j < length; j++) {
			// check if found
